...
M.run_check_job = function(tla_file_path, job_args, config)
  utils.check_filetype_is_tla(tla_file_path)
  vim.cmd('write')
  local command = config.java_executable
  local args = vim.tbl_flatten({ config.java_opts, job_args })
  local command_and_args = command .. " " .. table.concat(args, " ")
  if vim.fn.executable('tla-colorize.py') == 1 then
    RunCommandX("tla-colorize.py " .. command_and_args)
  else
    RunCommandX(command_and_args)
  end
end

local function RunCommandX(arg)
  vim.cmd('write')
  vim.cmd('noautocmd belowright split | terminal ' .. arg)
  vim.cmd('norm G')
  vim.cmd('startinsert')
end
